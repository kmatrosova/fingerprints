...
# %%
big_likes, big_users, big_items, big_user_items, big_item_users = get_data_from_csv('../data/users_bipartite_aligned_song_1000000.csv')
print('\nNumber or users:', len(big_users))
print('\nNumber or songs:', len(big_items))

# %%
# Info table
info_table = info_table(big_user_items, big_item_users)
fig = go.Figure(data=[go.Table(header=dict(values=['metric', 'favourite songs per user', 'fans per song']),
                 cells=dict(values=info_table))])
fig.show()

# %%
# Number of users who have liked x number of songs
nb_users_per_nb_items = distribution(big_user_items)
# Number of songs that have been liked by x number of users
nb_items_per_nb_users = distribution(big_item_users)

# %%
# Distibution of number of favourite songs per user
title = 'Distibution of number of favourite songs per user'
x_axe = 'nb_fav_songs'
y_axe = 'nb_users'
...
# %%
# Distibution of number of fans per song
title = 'Distibution of number of fans per song'
x_axe = 'nb_fav_songs'
y_axe = 'nb_users'
